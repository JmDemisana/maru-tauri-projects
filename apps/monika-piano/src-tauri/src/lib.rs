...
async fn move_to_next_monitor(window: tauri::WebviewWindow) -> Result<(), String> {
    ...
    #[cfg(not(target_os = "windows"))]
    {
        ...
        let _ = window.set_position(tauri::PhysicalPosition::new(mon_pos.x, mon_pos.y));
        let _ = window.set_size(tauri::PhysicalSize::new(mon_size.width, mon_size.height));
    }
...
pub fn run() {
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![move_to_next_monitor])
        .setup(|app| {
            if let Some(window) = app.get_webview_window("main") {
                let _ = window.set_resizable(false);
                let _ = window.set_maximizable(false);

                // Fit to primary work area (respects taskbar regardless of aspect ratio or taskbar position)
                #[cfg(target_os = "windows")]
        ...
        .expect("error while running Monika Piano application");
}
